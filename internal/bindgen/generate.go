...
func parseStructFields(node ClangASTNode) (string, bool) {
	...
	for _, child := range node.Inner {
		if child.Kind == "FieldDecl" {
			clioType, ok := mapCTypeToClio(child.Type.QualType)
			if !ok {
				return "", false // Skip structs with unmappable fields
			}
			fields = append(fields, fmt.Sprintf("  %s: %s", child.Name, clioType))
		}
	}
	if len(fields) == 0 {
		return "", false
...
func parseFunction(node ClangASTNode, seenStructs map[string]bool) (string, bool) {
	...
	retType, ok := mapCTypeToClio(strings.Split(node.Type.QualType, " (")[0])
	if !ok {
		return "", false
	}
	...
	for _, child := range node.Inner {
		if child.Kind == "ParmVarDecl" {
			pType, ok := mapCTypeToClio(child.Type.QualType)
			if !ok {
				return "", false
			}
...
func mapCTypeToClio(cType string) (string, bool) {
	...
	case "unsigned int", "unsigned long", "unsigned long long":
		return "int", true // Clio's int is 64-bit
	case "float":
		return "f32", true
	case "double":
		return "float", true
	...
	case "char", "signed char", "int8_t":
		return "u8", true // Or i8 if Clio has it
	case "short", "int16_t":
		return "int", true
